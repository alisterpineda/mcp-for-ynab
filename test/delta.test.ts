...
import { category, fullBudget, month, budgetDetail, subtransaction, transaction } from "./fixtures.js";

const NOW = new Date("2026-09-21T14:00:00Z");

describe("buildCache", () => {
  it("indexes every entity list by id and months by date", () => {
    const cache = buildCache(fullBudget(), 10, NOW);
  ...
  it("tolerates a budget with no lists at all", () => {
    const cache = buildCache(budgetDetail(), 1, NOW);
...
describe("applyDelta", () => {
  ...
  it("does not count deleting an unknown entity", () => {
    const cache = buildCache(fullBudget(), 10, NOW);
    const stats = applyDelta(cache, budgetDetail({ transactions: [transaction("never", "2026-01-01", 0, { deleted: true })] }), 11, NOW);
  ...
  it("removes a category from a month and removes whole months", () => {
    const cache = buildCache(fullBudget(), 10, NOW);
    applyDelta(cache, budgetDetail({ months: [month("2026-09-01", [category("c2", "Household", { deleted: true })])] }), 11, NOW);
    assert.deepEqual(cache.months["2026-09-01"].categories.map((c) => c.id), ["c1"]);

    applyDelta(cache, budgetDetail({ months: [month("2026-09-01", [], { deleted: true })] }), 12, NOW);
    assert.deepEqual(cache.months, {});
  });

  it("updates budget metadata such as a renamed budget", () => {
    const cache = buildCache(fullBudget(), 10, NOW);
    applyDelta(cache, budgetDetail({ name: "Household 2027", last_month: "2027-01-01" }), 11, NOW);
