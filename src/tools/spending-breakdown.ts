...
\`start\` and \`end\` accept \`YYYY-MM-DD\` or \`YYYY-MM\` (a month widens to its whole span) and are both inclusive; they default to the first of the current month and today. \`categories\`, \`groups\`, \`payees\` and \`accounts\` narrow the range and take ids or exact names — no need to look an id up first, and an ambiguous name comes back as an error listing the candidates. Values inside one list are ORed, and the lists are ANDed together.
  ...
  server.registerTool(
    "spending_breakdown",
    {
      ...
      inputSchema: {
        ...
        accounts: z.array(z.string()).optional().describe("Only these accounts, by id or exact name."),
...
function buildBreakdown(context: ToolContext, args: BreakdownArgs): Report {
  ...
  const filter: SpendingFilter = { from, to, ...resolveFilters(context, args) };
  ...
  const body: Report = {
    ...
    group_by: groupBy,
    total: context.money(-total.spent),
    lines: total.count,
    scope: SCOPE,
    excluded: context.db.spendingExclusions(budgetId, filter),
  };
